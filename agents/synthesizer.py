...
3. **必须对每一天的景点调用 amap_multi_route**。传入景点名称即可，工具已自动缓存 POI 坐标。
   若景点不在 POI 结果中，传入 `名称;lng,lat` 格式（分号前是名称，分号后是坐标）。
4. amap_route_plan 同上，传入名称即可，工具自动从缓存查找坐标。
5. 工具返回 "⚠️ 坐标异常提醒" 或 "⚠️ 存在异常长距离路段" 时，该路段数据不可用，用你的知识估算替代。
6. 工具返回错误时标注 "⚠️ 路线数据暂不可用"，不中断报告生成。
